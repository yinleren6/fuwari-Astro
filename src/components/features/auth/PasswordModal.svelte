...
<div class="password-protection">
  <div class="password-container card-base">
    ...
    {#if hint}
      <p class="hint-text">密码提示: {hint}</p>
    {:else}
      <p class="description">请输入密码查看此文章</p>
    {/if}

    <form class="password-form" onsubmit={handleSubmit}>
      <input type="password" id="password-input" placeholder="输入密码" class="password-input" bind:value={password} disabled={isLoading} autocomplete="off" />
      <button id="unlock-btn" class="unlock-button" type="submit" disabled={isLoading}>
        {isLoading ? "解锁中..." : "解锁"}
      </button>
    </form>
